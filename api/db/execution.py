...
from api.db.engine import UnknownDatabaseTargetError, get_engine
...
def execute_sql(sql: str, target: str = DEFAULT_TARGET) -> ExecutionResult:
    """Validate, then run one read-only query against one registered target.

    Args:
        sql: the candidate statement, as generated by the model.
        target: which database to run it against — a key into
            `api.targets.DATABASE_TARGETS`, never a raw connection string
            (Invariant #1 of the dynamic-database-switching feature). Every
            caller written before that feature omits this and gets
            `DEFAULT_TARGET`, unchanged.

    Returns:
        An `ExecutionResult`. Expected failures -- rejection, timeout, a bad
        column, an unreachable or unconfigured database -- are returned, not
        raised (AC16).
    ...
    **There is no way to skip validation** (AC2). No parameter, keyword,
    ...
    Never raises (AC23) — including on an unknown or unconfigured `target`.
    That case is expected to be refused earlier, at the HTTP boundary that
    can return a clean 400/503 before spending an agent turn on it, but this
    function does not trust that upstream check happened; a request-validity
    problem is categorised the same way an unreachable database already was,
    rather than escaping as a raised exception the loop was never built to
    catch.
    """
    ...
    # Resolving the target is its own, narrow try/except -- deliberately not
    # folded into the one around `_run` below. `get_engine(target)` can only
    # fail two ways, both about the target itself (`UnknownDatabaseTargetError`
    # or a `RuntimeError` for an unset DSN), and both happen before a
    # connection exists. A wider net here once caught a genuine `RuntimeError`
    # raised *inside* `_run` by unrelated code (`tests/test_execution.py::
    # test_ac6_rollback_happens_even_when_the_body_raises` deliberately injects
    # one) and reported it as an unreachable database instead of letting it
    # propagate -- the exact "a bare except swallows a real bug" shape `002`
    # already learned from `TokenError`, just with `RuntimeError` standing in
    # for `Exception`.
    try:
        engine = get_engine(target)
    except (UnknownDatabaseTargetError, RuntimeError) as exc:
        # Categorised identically to an unreachable database: from the
        # caller's side both mean "this database could not be reached," and
        # AC23 does not carve out an exception for a configuration problem.
        return ExecutionResult(
            ok=False,
            category=CATEGORY_CONNECTION_ERROR,
            error=f"Could not reach database target {target!r}: {exc}",
        )

    try:
        return _run(sql, engine)
...
def _run(sql: str, engine) -> ExecutionResult:
    ...
    Takes an already-resolved `Engine`, not a `target` string -- `execute_sql`
    resolves the target in its own try/except, so nothing in here can raise
    for a target-related reason, and a bug in `_harden` or the query itself
